...
First we examine if any of our data is highly correlated (or not); high correlation between variables is an issue causal inference models.
...
From these, I decided to keep log_DissAmmonia, log_Turbidity, Salinity, and Temperature in my model. I also added log_DissOrthophosphate since I think it's usually considered an important variable and the lag term (lag_logChlorophyll) to account for autocorrelation. Note that Temperature appears to have a non-linear, almost cubic relationship with log_Chlorophyll.

I'm also adding log_Salinity over Salinity, which is probably wrong, but it's useful for the SEM examples.
...
You can also visualize potential interaction effects between categorical and continuous variables (or cat:cat, though cont:cont is much more difficult; it's easier to choose those via model selection). If the relationship between the response and continuous predictor appears to change based on the categorical one, the interaction may be significant.
...
For simplicity, I subset the data from 02-01-2017 to 12-31-2022 to escape issues with unbalanced regional data. I also added three new variables: three basis splines for Temperature. This is so I correctly model Temperature's likely non-linear dependence on Chlorophyll; with three terms, I'm assuming it's cubic.
...
```{r}
plot(acf(residuals(fit_chl)))
plot(pacf(residuals(fit_chl)))
```
...
Model diagnostics look pretty good; slight autocorrelation at the ~17th lag indicates minor model misspecification, and there's some right-tail skew, but neither are particularly noteworthy. The lags do show some seasonality but it's not significant.
...
poly_obj <- attr(model.matrix(fit_chl)[,'Temp1'], 'coefs')
...
newdat$partial <- predict(fit_chl, newdata = newdat, re.form = NA)

newdat$partial_centered <- newdat$partial - mean(newdat$partial)

ggplot(newdat, aes(x = temp_seq, y = partial_centered)) +
  geom_line() +
  labs(
    x = 'Temperature',
    y = 'Partial Effect',
...
We note that the GAM more closely fits our original log_Chlorophyll ~ Temperature plot. We also note that our chi-square goodness of fit test is now significant, meaning this model (with a more correctly modeled Temperature) is a poorer fit, likely due to missing variables.
