...
df_monthly_rm <- df_monthly %>%
  select(-c(DissNitrate, DON, SJR_Inflow, Sac_Inflow, TKN, TotPhos))
```
...
df_monthly_log <- df_monthly_log %>%
  ...
  group_by(Region) %>%
  mutate(
    lag_logChlorophyll = dplyr::lag(log_Chlorophyll),
    lag_Chlorophyll = dplyr::lag(Chlorophyll),
    lag_logDissAmmonia = dplyr::lag(log_DissAmmonia),
  ) %>%
  ungroup() %>%
...
Looks okay! It does overestimate, but it's a consistent offset and not horribly off.

Next is model diagnostics:

**Root mean square error/mean absolute error/bias**
...
print(exp(RMSE)); print(exp(MAE)); print(exp(bias))
```

Based on MAE, model is off by \~40% on average. The RMSE says its off by \~ 53%, which indicates some larger errors. Based on bias, the model over-predicts by \~15%.
...
\~14% of the total variance is explained by between-region variance, which justifies the random intercept term.
...
Residuals look i.i.d., which is what we want (ie. randomly scattered about 0).
...
We see that no lags are statistically significant in any region, which is what we want (the few minor ones at higher lags can be ignored).
...
Based on Cook's distance there are no significant outliers.
...
Our data mostly follows the 1:1 line, indicating normality, with some slight deviations due to skew.
