...
```{r}
test <- df_pesp %>%
  filter(format(Date, '%Y-%m') == '2017-08') %>%
...
df_sum1_bio <- df_pesp %>%
  ...
  summarize(Total_Biovolume = sum(Biovolume_per_mL, na.rm = TRUE), .groups = 'drop')
...
df_sum2_bio <- df_sum1_bio %>%
  ...
  summarize(Station_Biovolume = mean(Total_Biovolume, na.rm = TRUE), .groups = 'drop')

# count unique stations per region-monthyear
station_count <- df_pesp %>%
...
df_meanbio <- df_sum2_bio %>%
  ...
  summarize(Sum_Biovolume = sum(Station_Biovolume, na.rm = TRUE), .groups = 'drop') %>%
  left_join(station_count, by = c('Region', 'MonthYear')) %>%
  mutate(Mean_Biovolume = Sum_Biovolume / n_stations,
...
df_pesp_mo %>%
  ...
  AlgalGroup = str_remove(column, '^bv_|^units_')) %>%
  filter(!is.na(metric)) %>%
...
Remove groups with little data
```{r}
df_pesp_mo <- df_pesp_mo %>%
  select(-matches('Raphidophytes|Ciliates|Coccolithophores|Ochrophytes|Xanthophytes|Eustigmatophytes|Dinoflagellates|Euglenoids|Chrysophytes'))
...
df_pesp_sea <- df_pesp_mo %>%
  add_season() %>%
  select(-Month, -MonthYear) %>%
  group_by(Region, Season, Year) %>%
  summarize(across(where(is.numeric), ~na_if(mean(., na.rm = TRUE), NaN)), .groups = 'drop')
```
