...
```{r remove extra dfs for memory efficiency}
rm(df_nutr_c1, df_nutr_c2, df_nutr_c3, df_nutr_c4, df_nutr_c5, df_frp_phyto_coord)
rm(df_nutr_daily_dups_fixed, df_nutr_daily_dups)
rm(df_nutr_dt_dups, df_nutr_dt_dups_fixed, df_nutr_dups_nodt, df_nutr_dups_nodt_fixed)
rm(df_nutr)
```
...
df_dwq_c2 <- df_dwq_c1 %>%
  ...
  mutate(Parameter = if_else(Parameter == "Microcystis", "MVI", Parameter))
```
...
```{r remove extra dfs for memory efficiency}
rm(df_dwq_c1, df_dwq_c2, df_dwq_c3, df_dwq_c4, df_dwq_c5, df_dwq_c6, df_dwq_c7)
rm(df_dwq_daily_dups_fixed, df_dwq_daily_dups)
rm(df_dwq_turb_dups, df_dwq_turb_dups_fixed)
rm(df_dwq_dt_dups, df_dwq_dt_dups_fixed)
rm(df_dwq)
```
...
```{r remove extra dfs for memory efficiency}
rm(df_usgs_clams, df_usgs_clams_biomass, df_usgs_clams_c, df_usgs_clams_rates, ndf_usgs_clams)
rm(df_smscg_clams, df_smscg_clams_c)
rm(df_drt_clams, ls_drt_clams)
rm(df_clams_all, df_clams_all_c)
```
...
df_sum1_bio <- df_pesp %>%
  ...
  summarize(total_biovol = sum(Biovolume_per_mL, na.rm = TRUE), .groups = 'drop')
...
df_meanbio <- df_sum2_bio %>%
  ...
  summarize(sum_biovol = sum(station_biovol, na.rm = TRUE), .groups = 'drop') %>%
  left_join(station_count_bio, by = c('Region', 'MonthYear')) %>%
  mutate(Mean_Biovolume = sum_biovol / n_stations,
...
df_meanunits <- df_sum2_units %>%
  ...
  mutate(Mean_Units = sum_units / n_stations,
         MonthYear = format(MonthYear, '%b-%Y'))
...
```{r remove extra dfs for memory efficiency}
rm(df_sum1_bio, df_sum2_bio, df_sum1_cbio, df_sum2_cbio, df_sum1_units, df_sum2_units)
rm(df_meanbio, df_meanbio_mo, df_meanc, df_meanc_mo, df_meanunits, df_meanunits_mo)
rm(station_count_bio, station_count_c, station_count_units, df_pesp)
```
...
other_groups <- c(
  'Raphidophytes', 'Ciliates', 'Coccolithophores', 'Ochrophytes',
  'Xanthophytes', 'Eustigmatophytes', 'Dinoflagellates',
  'Euglenoids', 'Chrysophytes'
)
...
```{r remove extra dfs for memory efficiency}
rm(df_clams_avg_mo, df_clams_avg_seas, df_dayflow_tot_mo, df_dayflow_tot_seas, df_pesp_mo, df_pesp_seas)
rm(df_veg_index_avg_mo, df_veg_index_avg_seas, df_wind_mo, df_wyt_2010_2024_mo, df_wyt_2010_2024_seas)
rm(ls_dwq_avg_wide, ls_nutr_avg_wide, df_yr_mo_reg, df_yr_seas_reg)
```
