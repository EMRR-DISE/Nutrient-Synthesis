...
df_filledhab <- station_months %>%
  ...
  mutate(
    hab_biovol_station = replace_na(hab_biovol_station, 0),
    mc_biovol_station = replace_na(mc_biovol_station, 0)
  ) %>%
  summarize(
...
# FIX: add in PESP real quick

```{r}
df_regional <- readRDS(here("data/processed/monthly_values.rds"))

df_filledhab <- df_filledhab %>%
  filter(!(Year == 2024 & Month > 6)) %>%
  # Create Month-Year column
  mutate(
    MonthYear = paste(month(Month, label = TRUE), Year, sep = "-"),
...
df_final <- df_regional %>%
  left_join(df_filledhab, by = c('MonthYear', 'Year', 'Month', 'Region')) %>%
  rename(MVI = Microcystis)

saveRDS(df_final, 'data/processed/monthly_values_withpesp.rds')
```
